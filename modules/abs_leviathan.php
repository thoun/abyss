...
class LeviathanManager {
  ...
  public static function typedLeviathans(array $dbResults, $game) {
  ...
  public static function draw($game, int $slot) {
    $leviathan = new Leviathan(self::$game->getObject("SELECT * FROM leviathan WHERE place = 0 ORDER BY RAND() LIMIT 1"), $game->LEVIATHANS);
  ...
  public static function getVisibleLeviathans($game) {
    return self::typedLeviathans(self::$game->getCollection( "SELECT * FROM leviathan WHERE place > 0"), $game);
  }
